    ...
    public function createGoogleProductFeeds()
    {
        ...
        try {
            ...
            foreach($channels as $channel){
                ...
                $feeds[$path] = (new \JBAShop\Services\GoogleProductsFeed($path))->generateFeeds($channel->get('slug') . '_products');
            }
            foreach($feeds as $key => $feed){
                //Compress feeds if any
                (new \JBAShop\Services\GoogleProductsFeed($key))->compressFeeds($feeds);
            }
