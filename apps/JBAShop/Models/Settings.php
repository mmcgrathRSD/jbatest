...
class Settings extends \Shop\Models\Settings
{
    ...
    * */
    ...
    public function getShippingMultiplier($id, $title = null){
    	
		$id = str_replace('.', '-', $id);
		$base = $this->{'shipping_multiplier.base'};
		$others = $this->{'shipping_multiplier.others'};
		$ids = array_keys((array)$others);
		$multiplier = $base;

		if( empty( $ids ) ){ // no custom multipliers so far so add this one
			if( $title == null ){
				$title = $id;
			}
			
			$this->{'shipping_multiplier.others.'.$id} = array( 'title' => $title, 'multiplier' => $base );
		} else {
			$found = false;
			 // does this method exist?
			foreach($ids as $key){
				if( $key == $id ){ // here it is so remember its multiplier
					$found = true;
					$multiplier = $others[$key]['multiplier'];
				}
			}
			
			if(!$found){ // nope, it does not exist so add it with base multiplier
				if( $title == null ){
					$title = $id;
				}
				$others[$id] = array('title' => $title, 'multiplier' => $base );
				$this->{'shipping_multiplier.others'} = $others;
			}
		}
